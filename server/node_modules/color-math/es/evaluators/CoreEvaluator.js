...
function (_EvaluatorBase) {
  ...
  _createClass(CoreEvaluator, [{
    ...
    value: function evalArrayElement(node) {
      var array = Utils.forceType(node.obj.evaluate(this), ValueType.Array, node.obj.$loc);
    ...
    value: function evalScale(node) {
      ...
      var colors = Array.isArray(node.colors) ? node.colors.map(function (expr) {
        return Utils.forceType(expr.evaluate(_this5), ValueType.Color, expr.$loc);
      ...
      if (node.domain !== void 0) {
        var domain = node.domain.map(function (expr) {
          return Utils.forceType(expr.evaluate(_this5), ValueType.Number, expr.$loc);
        });
        scaleParams.push({
          name: 'domain',
          value: domain
    ...
    value: function evalBezier(node) {
      var colors = Utils.forceType(node.colors.evaluate(this), ValueType.ColorArray, node.colors.$loc);
      var colorsMin = 2;
      var colorsMax = 5;
    ...
    value: function evalUnaryMinus(node) {
      var value = Utils.forceType(node.value.evaluate(this), ValueType.Number, node.value.$loc);
      value = -value;
      return value;
    }
  }, {
    ...
    value: function evalColorInverse(node) {
      var value = Utils.forceType(node.value.evaluate(this), ValueType.Color, node.value.$loc);
    ...
    value: function evalCorrectLightness(node) {
      var value = Utils.forceType(node.value.evaluate(this), ValueType.ColorScale, node.value.$loc);
      value = value.clone();
    ...
    value: function evalColorsMix(node) {
      ...
      var ratio = ratioExpr ? Utils.forceType(ratioExpr.evaluate(this), ValueType.Number, ratioExpr.$loc) : void 0;
    ...
    value: function evalManageColorAlpha(node) {
      ...
      if (node.value === void 0) {
        ...
        var value = ['*', '/'].includes(node.operator) ? Utils.forceType(node.value.evaluate(this), ValueType.Number, node.value.$loc) : Utils.forceNumInRange(node.value.evaluate(this), 0, 1, node.value.$loc);
    ...
    value: function evalSetColorScalePadding(node) {
      ...
      value = Array.isArray(value) ? Utils.forceRange(value, node.value.$loc) : Utils.forceType(value, ValueType.Number, node.value.$loc);
    ...
    value: function evalSetScaleDomain(node) {
      var value = Utils.forceType(node.value.evaluate(this), ValueType.NumberArray, node.value.$loc);
...
export { CoreEvaluator as default };