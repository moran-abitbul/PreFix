...
export function blendColors(bg, fg, mode) {
  ...
  const resComps = [void 0, void 0, void 0, fgComps[3] + bgComps[3] * (1 - fgComps[3])]
  ...
  for (let i = 0; i < 3; i++) {
    ...
    if (resComps[3]) {c = (fgComps[3] * c2 + bgComps[3] * (c1 - fgComps[3] * (c1 + c2 - c))) / resComps[3]}
...
export function cmyToCmykArray(valuesOrC, m, y) {
  ...
  if (!Array.isArray(valuesOrC)) {c = valuesOrC} else {[c, m, y] = valuesOrC}
  ...
  if (types.every(t => (t & valueType) !== t)) {
    const strs = types.map(t => {
      switch (t) {
        ...
        default:                    throw new Error('invalid value type')
      }
    })
